...
public class User extends ExecuteDB
{
	...
	public boolean add()
	{
		...
		this.strSql=this.strSql + "(uname,name,pwd,sex,email,address,tel,birth) ";
		this.strSql=this.strSql + "values('" + this.uname + "','" + this.name + "','" + this.pwd + "','" + this.sex + "','" + this.email + "','" + this.address + "','" + this.tel + "','" + this.birth + "')";				
		//",'" + this.post + "','" + this.safequestion + "','" + this.safeanswer + "','" + this.role + "','" + 
		//this.money + 
		//"')";
	...
	{
		//this.strSql="select username,role from `users` ";
	...
	{
		this.strSql="select stuid,stu_name,sex,train_date,school,grade,class_name,hui_flag from `admin_stu` order by stuid asc";
		ResultSet rs = null;
		try
		{
	...
	//	this.strSql="select userid,sex,uname,name,address,email,tel,money from `users` where role= 3 order by userid asc";
		this.strSql="select userid,sex,uname,name,address,email,tel,money from `users` order by userid asc";
		ResultSet rs = null;
		try
		{
			rs = super.exeQuery(this.strSql);
