	...
	<body bgcolor="#F2F2F2">
		...
		%>
			    ...
			    <td width="9%" align="center">E-mail:<%=sData[5]%></td>
					...
					<td width="5%" height="25" align="center">&nbsp;<a href="updata_user_form.jsp?UserID=<%=sData[0]%>&showPage=<%=showPage%>" target="_self"><font color="#0000FF">修改</font></a></td>
		...
		<p><input name="sub" type="submit" class="input1" value="删除选中的员工">&nbsp;&nbsp;
		<input name="btn" type="button" class="input1" onClick="javascript:window.location='add_our_users.jsp'" value="添加新员工">
		</p>
		</form>
		<p>
		<form action="show_our_users.jsp" method="post" target="_self">	
			...
			<a href="show_our_users.jsp?showPage=1" target="_self"><font color="#0000FF">[首页]</font></a>&nbsp;			
			<%
				//判断"上一页"链接是否要显示
				if(showPage > 1)
				{				
			%>
					<a href="show_our_users.jsp?showPage=<%=showPage-1%>" target="_self"><font color="#0000FF">[上一页]</font> </a>&nbsp;
  <%
				}
				else
				{
				...
				{				
			%>
					<a href="show_our_users.jsp?showPage=<%=showPage+1%>" target="_self"><font color="#0000FF">[下一页]</font> </a>&nbsp;
		  <%
				}
				else
				{
			...
			%> 
			<a href="show_our_users.jsp?showPage=<%=pages.getPageCount()%>" target="_self"><font color="#0000FF">[尾页]</font> </a>&nbsp;
			转到
